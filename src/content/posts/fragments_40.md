---
title: Workers + KV + WebDAV 搭一个临时文件中转站（附完整代码）
pubDate: 2025-10-17
categories: ['笔记']
description: '利用 Cloudflare Workers KV 的边缘存取能力 + 闲置 WebDAV 空间，做一个不落本地、跨设备的临时文件传输服务，并附完整 Worker 代码便于直接部署。'
...
提示：代码里有两处标了“部署时请替换”的占位符（比如域名），按你的实际情况替换即可。

<details>
<summary><strong>▶ 点击展开完整 Worker 代码（折叠展示，不影响阅读）</strong></summary>
